...
class MyRndException_32 extends Exception { // 커스텀 예외를 Exception 클래스를 상속하여 정의한다.
}
...
class Get_m_3 { // get_n_3형 객체를 객체 지시자로 받아들여 추가로 모듈러 연산 값이 연산일때 오류 처리를 위한 클래스
    ...
    Get_m_3(Get_n_3 gn){ // 객체 생성자의 파라미터로 Get_n_1형 객체 gn을 받아들인다.
        this.gn = gn; // 그 객체의 주소를 이 클래스의 필드에 저장
    }

    int getmod() throws MyRndException_31{ // 모듈러 연산을 추가로 진행하여 그 결과값이 0이면 오류 반환, 아니면 그 값 반환하는 메소드 선언
        ...
        try { // 예외가 발생할 것 같은 구문을 try문 안에 넣고
            ...
            if((m = (x % y)) == 0){ // m x와 y를 모듈러 연산한 결과가 0이면
                throw new MyRndException_32();
            }
        } catch(MyRndException_32 e) { // throw한 예외를 받아줄 catch 문 작성
            System.out.println("MyRndException_32"); // catch 했을때 할 행동인 MyRndException_32 출력하기
