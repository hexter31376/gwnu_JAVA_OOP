...
class IO_71 {
    void io_test () {
        ...
        Student st1 = new Student("Lee", 19); // 학생 정보 생성
        Student st2 = new Student("Kim", 18); // 학생 정보 생성
        Student st3 = new Student("Park", 20); // 학생 정보 생성

        try {
            raf.writeUTF(st1.getName()); // 학생 클래스의 이름을 writeUTF 메소드로 작성
            raf.writeInt(st1.getAge()); // 학생 클래스의 나이를 writeInt 메소드로 작성
            raf.writeUTF(st2.getName()); // 학생 클래스의 이름을 writeUTF 메소드로 작성
            raf.writeInt(st2.getAge()); // 학생 클래스의 나이를 writeInt 메소드로 작성
            raf.writeUTF(st3.getName()); // 학생 클래스의 이름을 writeUTF 메소드로 작성
            raf.writeInt(st3.getAge()); // 학생 클래스의 나이를 writeInt 메소드로 작성
...
class IO_72 {
    void io_test () {
        ...
        try {
            ...
            st = new Student(raf.readUTF(), raf.readInt()); // seek로 읽어온 커서 위치부터 학생 이름과 나이를 st 객체 지시자에 순서대로 저장
            System.out.println("3rd: " + st.getName() + ", " + st.getAge()); // 해당 학생 이름과 이름 출력

            raf.seek((7 + 4) * 0); // Student의 name(기본 5바이트 + utf로 인한 길이 정보 2바이트)과 age(int 4바이트)를 읽기 위해 seek 메소드를 이용합니다.
            st = new Student(raf.readUTF(), raf.readInt()); // seek로 읽어온 커서 위치부터 학생 이름과 나이를 st 객체 지시자에 순서대로 저장
            System.out.println("1rd: " + st.getName() + ", " + st.getAge()); // 해당 학생 이름과 이름 출력

            raf.seek((7 + 4) * 1); // Student의 name(기본 5바이트 + utf로 인한 길이 정보 2바이트)과 age(int 4바이트)를 읽기 위해 seek 메소드를 이용합니다.
            st = new Student(raf.readUTF(), raf.readInt()); // seek로 읽어온 커서 위치부터 학생 이름과 나이를 st 객체 지시자에 순서대로 저장
            System.out.println("2rd: " + st.getName() + ", " + st.getAge()); // 해당 학생 이름과 이름 출력
